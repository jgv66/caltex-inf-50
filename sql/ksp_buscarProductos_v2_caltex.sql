-- update 01/10/2018 -> cambia criterio de 'con compras' ya que existen calzados con stock y ademas con compras futuras...
-- update 20/08/2018 -> incorpora columna codigosincolor, columna de ecuacion de desc.max. y se resuelve en el cliente

-- exec ksp_buscarProductos_v2_caltex '1026','','001','01P','','','','','DESCRIPCION',0,true,false,false,'02','HIC',''
-- exec ksp_buscarProductos_v2_caltex '','nw dma','001','01P','','','','','DESCRIPCION',0,true,false,false,'02','HIC','09893161' ;
...
BEGIN
	...
	--
	if ( @cliente <> '' ) begin
		select * from PDIMCLI with (nolock) WHERE CODIGO=@cliente ;
	end;
	--
	select @sucursal = KOSU from TABBO with (nolock) where EMPRESA=@empresa AND KOBO = @bodega;
	--
	set @query += 'with precios as ( select L.KOPR,TL.MELT,(case when TL.MELT='+char(39)+@letraN+char(39)+' then '+char(39)+@neto+char(39)+' else '+char(39)+@bruto+char(39)+' end) as tipolista,';
